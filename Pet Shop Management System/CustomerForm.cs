...
namespace Pet_Shop_Management_System
{
    public partial class CustomerForm : Form
    {
        ...
        private void dgvCustomer_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                CustomerModule module = new CustomerModule(this);
                module.lbluid.Text = dgvCustomer.Rows[e.RowIndex].Cells[1].Value.ToString();
            ...
            {
                if (MessageBox.Show("Are you sure you want to delete customer record?", "Delete Record", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
        ...
        public void LoadCustomer()
        {
            ...
            cm = new SqlCommand("SELECT * FROM tbCustomer WHERE CONCAT(name,address,phone) LIKE '%" + searchCustomer.Text + "%'", cn);
            cn.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
